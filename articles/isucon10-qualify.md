...
これもINDEXを貼れば解決されますが、落とし穴としてMySQL 5.7では降順INDEXがサポートされていません。
...
SPATIAL INDEXを貼ることで、バウンディングボックスによる絞り込み (`latitude <= ? AND latitude >= ? AND ...`) も不要になり、高速化が図れます。
...
そのため、当日マニュアルを見て実装するだけではパフォーマンスが改善されないようになっています。
計測してからのチューニングが大切であることを示すためのボトルネックでした。

しかし、運営側の不手際によって競技開始直後にベンチマークが実行できない状況にありました。
これにより、とりあえずボットを弾く処理から手をつけた競技者も多かったのではないでしょうか。

作問時は、アプリケーションのボトルネックとしてボットに対する正規表現マッチが重くなることを案として考えていました。
実はボットのUser-Agentは、走行しているボット1台に対して固定で設定されていました。
User-Agentとボット判定の結果をキャッシュすることでアプリケーション側のボトルネックを解消できると考えていました。
しかし、データベースのボトルネックが大きすぎたためにボット判定がボトルネックになることはありませんでした。
...
また、イス・物件検索の検索条件はレベルが高ければ高いほど複雑になる仕様となっています。
...
想定解としては、椅子の最短の辺がドアの短い方の辺より短く、椅子の2番目に短い辺がドアの長い方の辺より短いことを調べることでした。
